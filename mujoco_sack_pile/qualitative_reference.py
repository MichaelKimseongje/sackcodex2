...
@dataclass
class ReferenceShellSpec:
    """정량 benchmark가 아닌 보조 시각 점검용 shell proxy 초기 조건."""
...
class QualitativeReferenceGenerator:
    """정량 평가가 아닌 shell-based qualitative reference scene을 생성한다."""
    ...
    def generate(self, seed: int, episode_id: str, sack_count: int | None = None) -> Path:
        ...
        xml_path = self.generated_dir / f"{episode_id}_reference_shell.xml"
        xml_path.write_text(self._build_xml(episode_id, sacks), encoding="utf-8")
        return xml_path

    def _sample_specs(self, rng: random.Random, count: int) -> list[ReferenceShellSpec]:
        families = [
    ...
    def _build_xml(self, episode_id: str, sacks: list[ReferenceShellSpec]) -> str:
        sack_xml = "\n".join(self._sack_xml(spec) for spec in sacks)
        return f"""<mujoco model="{episode_id}_reference_shell">
...
"""
    ...
    def _sack_xml(self, spec: ReferenceShellSpec) -> str:
        r, g, b, _ = spec.rgba
        payload_scale = spec.scale / 0.070
        # 이 경로는 실제 재질 fidelity를 주장하기 위한 것이 아니라,
        # 표면 shell proxy와 내부 payload proxy의 정성적 settling 장면을 빠르게 보는 용도다.
        payload = f"""
    ...
    <body name="{spec.name}" pos="{spec.pos[0]:.4f} {spec.pos[1]:.4f} {spec.pos[2]:.4f}" euler="{spec.euler[0]:.4f} {spec.euler[1]:.4f} {spec.euler[2]:.4f}">
      ...
      <flexcomp name="{spec.name}_shell_proxy" type="mesh" file="../../object/{spec.mesh_file}" dim="2" mass="{spec.mass:.4f}" radius="{spec.radius:.5f}" scale="{spec.scale:.5f} {spec.scale:.5f} {spec.scale:.5f}">
        <edge damping="0.18"/>
