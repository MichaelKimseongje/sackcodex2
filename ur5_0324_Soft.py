...
class DualUR5EEGuiIK:
    def __init__(self,
        ...
        p.setPhysicsEngineParameter(
            ...
            numSubSteps=5, # soft sack + eccentric ballast is much calmer with more substeps
        ...
        self.homeL = [0.3563, -1.3068, 1.6666, -1.9306, -1.5708, 0.2280]
        ...
        self.scale = 0.15
        self.mass_each = 0.20  # legacy sheet mass parameter

        sackpath = r"D:/Michael/2025/01.Research/01.Parceldetection/16.Pybullet/bullet3-master (1)/bullet3-master/examples/pybullet/examples/DeformableTest/Sackcodex2/object/sack8.obj"
        sackjson = r"D:/Michael/2025/01.Research/01.Parceldetection/16.Pybullet/bullet3-master (1)/bullet3-master/examples/pybullet/examples/DeformableTest/Sackcodex2/object/sack_rest_shape8.json"
        self.sack_obj_path = resolve_existing_path([
            os.path.join(self.base_dir, "object", "sack8.obj"),
            os.path.join(self.base_dir, "Sackcodex2", "object", "sack8.obj"),
            sackpath,
        ])
        self.sack_rest_json_path = resolve_existing_path([
            os.path.join(self.base_dir, "object", "sack_rest_shape8.json"),
            os.path.join(self.base_dir, "Sackcodex2", "object", "sack_rest_shape8.json"),
            sackjson,
        ])

        self.sack_scale = 0.07
        self.sack_total_mass = 3.6
        self.sack_soft_mass = 0.55
        self.sack_ballast_total_mass = self.sack_total_mass - self.sack_soft_mass
        self.sack_mass = self.sack_total_mass
        self.sack_rgba = [0.90, 0.85, 0.70, 0.92]
        self.sack_spawn_orn = p.getQuaternionFromEuler([0.0, math.pi / 2.0, 0.0])
        self.sack_spawn_pos = [0.50, 0.17, 0.24]
        self.sack_collision_margin = 0.003
        self.sack_friction = 1.0
        self.sack_use_neo = False
        self.sack_spring_stiffness = 75.0
        self.sack_spring_damping = 0.26
        self.sack_neo_mu = 80.0
        self.sack_neo_lambda = 120.0
        self.sack_neo_damping = 0.05
        self.sack_ballast_count = 1
        self.sack_ballast_radius = 0.022
        self.sack_ballast_linear_damping = 0.999
        self.sack_ballast_angular_damping = 0.999
        self.sack_ballast_rgba = [0.8, 0.1, 0.1, 0.0]
        self.soft_grasp_pad_target = np.array([0.50, 0.17, 0.33], dtype=np.float32)
        self.soft_grasp_pose_rpy = [math.pi, -math.pi / 2.0, -math.pi / 2.0]
        self.soft_grasp_opening = 0.48
        self.soft_grasp_closed_opening = 0.68
        self.soft_grasp_release_opening = 0.18
        self.soft_grasp_drop_below_pad = 0.030
        self.soft_anchor_count_per_pad = 0
        self.soft_anchor_center_count = 0
        self.soft_anchor_ids = []
        self.soft_ballast_anchor_ids = []
        self.top_band_enabled = True
        self.top_band_length = 0.050
        self.top_band_thickness = 0.010
        self.top_band_depth = 0.016
        self.top_band_mass = 0.55
        self.top_band_friction = 10.0
        self.top_band_linear_damping = 0.95
        self.top_band_angular_damping = 0.99
        self.top_band_anchor_count = 6
        self.top_band_mid_anchor_count = 4
        self.top_band_rgba = [0.55, 0.32, 0.14, 1.0]
        self.top_band_id = None
        self.top_band_anchor_ids = []
        self.top_band_grasp_constraint_id = None
        self.top_band_grasp_parent_link = None
        self.top_band_grasp_attach_distance = 0.035

        self.sack_rest_data = self._load_rest_shape_metadata(self.sack_rest_json_path)
        self.sack_top_indices = list(self.sack_rest_data["Top_ring"]["indices"])
        self.sack_mid_indices = list(self.sack_rest_data["Mid_ring"]["indices"])
        self.sack_bot_indices = list(self.sack_rest_data["Bot_ring"]["indices"])
        self.sack_rest_height = float(self.sack_rest_data["rest_height"]) * self.sack_scale
        ...
        self.sack_rest_vertices_scaled = self.sack_local_vertices * self.sack_scale

        self.handle_enabled = False
        self.handle_proxy_enabled = False
        self.handle_grasp_assist_enabled = False
        ...
        self.sack_content_constraint_ids = []

        self._initialize_left_soft_grasp_pose()
        self.sack_id = self._create_soft_sack_stage2()
        self.sack_content_ids = self._spawn_soft_eccentric_ballast()
        self.top_band_id = self._create_top_grasp_band()
        self._anchor_soft_sack_to_top_band()
        self._close_left_gripper_on_top_band()
        ...
        self.targetL = self._get_ee_pose(self.urL, self.eeL)
        self.targetR = self._get_ee_pose(self.urR, self.eeR)
        self.filtL = np.array(self.targetL[0] + self.targetL[1], dtype=np.float32)
        self.filtR = np.array(self.targetR[0] + self.targetR[1], dtype=np.float32)
        self.prev_L_ee = np.array(self.filtL, dtype=np.float32)
    ...
    def _release_top_band_grasp(self):
        cid = getattr(self, 'top_band_grasp_constraint_id', None)
        if cid is not None:
            try:
                p.removeConstraint(int(cid))
            except Exception:
                pass
        self.top_band_grasp_constraint_id = None
        self.top_band_grasp_parent_link = None

    def _release_soft_grasp_anchors(self):
        self._release_top_band_grasp()
        for cid in getattr(self, 'soft_anchor_ids', []):
            try:
                p.removeConstraint(int(cid))
            except Exception:
                pass
    ...
    def set_left_gripper_opening(self, opening):
        ...
        if target <= float(getattr(self, 'soft_grasp_release_opening', -1.0)):
            if getattr(self, 'soft_anchor_ids', None):
                self._release_soft_grasp_anchors()
            self._release_top_band_grasp()
    ...
    def _reset_arm(self, rid, joint_ids, q):
        ...
        # Keep freshly reset arms in position control so a grasped soft sack does not drag them away
        # before the higher-level pendant loop starts sending commands.
        for jid, qi in zip(joint_ids, q):
            p.setJointMotorControl2(
                rid,
                jid,
    ...
    def _load_rest_shape_metadata(self, json_path):
        with open(json_path, "r", encoding="utf-8") as fh:
            return json.load(fh)

    def _sample_indices_even(self, indices, count):
        seq = list(indices)
        if count <= 0 or len(seq) == 0:
            return []
        if len(seq) <= count:
            return [int(i) for i in seq]
        sample_pos = np.linspace(0, len(seq) - 1, count, dtype=np.int32)
        return [int(seq[i]) for i in sample_pos]

    def _initialize_left_soft_grasp_pose(self):
        self.soft_grasp_pad_target = self._left_gripper_pad_center_world().copy()
        self.set_left_gripper_opening(float(self.soft_grasp_opening))
        for _ in range(20):
            p.stepSimulation()

    def _create_soft_sack_stage2(self):
        top_center_local = np.array(self.sack_rest_data["Top_ring"]["center"], dtype=np.float32) * self.sack_scale
        pad_center = self._left_gripper_pad_center_world()
        desired_top_center = np.array(pad_center, dtype=np.float32) + np.array([0.0, 0.0, -self.soft_grasp_drop_below_pad], dtype=np.float32)
        rot = np.array(p.getMatrixFromQuaternion(self.sack_spawn_orn), dtype=np.float32).reshape(3, 3)
        base_pos = desired_top_center - rot.dot(top_center_local)
        self.sack_spawn_pos = base_pos.tolist()

        kwargs = dict(
            basePosition=self.sack_spawn_pos,
            baseOrientation=self.sack_spawn_orn,
            scale=float(self.sack_scale),
            mass=float(self.sack_soft_mass),
            useSelfCollision=1,
            frictionCoeff=float(self.sack_friction),
            collisionMargin=float(self.sack_collision_margin),
        )
        if self.sack_use_neo:
            kwargs.update(dict(
                useNeoHookean=1,
                NeoHookeanMu=float(self.sack_neo_mu),
                NeoHookeanLambda=float(self.sack_neo_lambda),
                NeoHookeanDamping=float(self.sack_neo_damping),
                useBendingSprings=1,
                useMassSpring=0,
            ))
        else:
            kwargs.update(dict(
                ...
                useBendingSprings=1,
                useMassSpring=1,
                springElasticStiffness=float(self.sack_spring_stiffness),
                springDampingStiffness=float(self.sack_spring_damping),
                springDampingAllDirections=1,
            ))
        soft_id = p.loadSoftBody(self.sack_obj_path, **kwargs)
        p.changeVisualShape(soft_id, -1, rgbaColor=self.sack_rgba)
        return soft_id

    def _create_top_grasp_band(self):
        left_pad = self._find_link_or_none(self.urL, ["left_inner_finger_pad", "left_inner_finger"])
        right_pad = self._find_link_or_none(self.urL, ["right_inner_finger_pad", "right_inner_finger"])
        if left_pad is None or right_pad is None:
            return None
        left_pos = np.array(p.getLinkState(self.urL, left_pad, computeForwardKinematics=True)[4], dtype=np.float32)
        right_pos = np.array(p.getLinkState(self.urL, right_pad, computeForwardKinematics=True)[4], dtype=np.float32)
        center = 0.5 * (left_pos + right_pos)
        span = right_pos - left_pos
        band_half = [0.5 * float(self.top_band_thickness), 0.5 * float(self.top_band_length), 0.5 * float(self.top_band_depth)]
        col = p.createCollisionShape(p.GEOM_BOX, halfExtents=band_half)
        vis = p.createVisualShape(p.GEOM_BOX, halfExtents=band_half, rgbaColor=self.top_band_rgba)
        bid = p.createMultiBody(
            baseMass=float(self.top_band_mass),
            baseCollisionShapeIndex=col,
            baseVisualShapeIndex=vis,
            basePosition=center.tolist(),
            baseOrientation=self._quat_from_y_axis(span),
        )
        p.changeDynamics(
            bid, -1,
            lateralFriction=float(self.top_band_friction),
            spinningFriction=0.25,
            rollingFriction=0.0,
            restitution=0.0,
            linearDamping=float(self.top_band_linear_damping),
            angularDamping=float(self.top_band_angular_damping),
            contactProcessingThreshold=0.0,
            ccdSweptSphereRadius=0.004,
        )
        try:
            p.setCollisionFilterPair(self.sack_id, bid, -1, -1, 0)
        except Exception:
            pass
        for content_id in getattr(self, 'sack_content_ids', []):
            try:
                p.setCollisionFilterPair(content_id, bid, -1, -1, 0)
            except Exception:
                pass
        return bid

    def _anchor_soft_sack_to_top_band(self):
        self.top_band_anchor_ids = []
        if self.top_band_id is None:
            return
        ...
        anchor_nodes = []
        anchor_nodes.extend(self._sample_indices_even(self.sack_top_indices, int(self.top_band_anchor_count)))
        mid_count = int(getattr(self, 'top_band_mid_anchor_count', 0))
        if mid_count > 0:
            upper_mid = []
            for node in self.sack_mid_indices:
                if verts[int(node), 2] >= float(verts[:, 2].mean()):
                    upper_mid.append(int(node))
            if not upper_mid:
                upper_mid = list(map(int, self.sack_mid_indices))
            anchor_nodes.extend(self._sample_indices_even(upper_mid, mid_count))
        ...
        for node in anchor_nodes:
            ...
            if node in used:
                continue
            used.add(node)
            cid = p.createSoftBodyAnchor(self.sack_id, node, self.top_band_id, -1)
            self.top_band_anchor_ids.append(cid)

    def _try_create_top_band_grasp(self):
        if self.top_band_id is None:
            return False
        if getattr(self, 'top_band_grasp_constraint_id', None) is not None:
            return True
        left_pad = self._find_link_or_none(self.urL, ["left_inner_finger_pad", "left_inner_finger"])
        right_pad = self._find_link_or_none(self.urL, ["right_inner_finger_pad", "right_inner_finger"])
        parent_link = int(self.plateL if self.plateL is not None else self.eeL)
        band_pos, band_orn = p.getBasePositionAndOrientation(self.top_band_id)
        band_pos = np.array(band_pos, dtype=np.float32)
        grip_center = self._left_gripper_pad_center_world()
        dist = float(np.linalg.norm(grip_center - band_pos))
        cL = len(p.getContactPoints(bodyA=self.urL, bodyB=self.top_band_id, linkIndexA=left_pad)) if left_pad is not None else 0
        cR = len(p.getContactPoints(bodyA=self.urL, bodyB=self.top_band_id, linkIndexA=right_pad)) if right_pad is not None else 0
        if (cL + cR) <= 0 and dist > float(self.top_band_grasp_attach_distance):
            return False
        parent_state = p.getLinkState(self.urL, parent_link, computeForwardKinematics=True)
        parent_pos, parent_orn = parent_state[4], parent_state[5]
        inv_pos, inv_orn = p.invertTransform(parent_pos, parent_orn)
        grip_center_local, _ = p.multiplyTransforms(inv_pos, inv_orn, grip_center.tolist(), [0, 0, 0, 1])
        _, parent_frame_orn = p.multiplyTransforms(inv_pos, inv_orn, band_pos.tolist(), band_orn)
        cid = p.createConstraint(
            self.urL, parent_link,
            self.top_band_id, -1,
            p.JOINT_FIXED,
        ...
        self.top_band_grasp_constraint_id = cid
        self.top_band_grasp_parent_link = parent_link
        return True

    def update_top_band_grasp(self):
        if self.top_band_id is None:
            return
        if float(getattr(self, 'left_gripper_target', 0.0)) <= float(getattr(self, 'soft_grasp_release_opening', -1.0)):
            self._release_top_band_grasp()
            return
        self._try_create_top_band_grasp()

    def _close_left_gripper_on_top_band(self):
        if self.top_band_id is None:
            return
        parent_link = int(self.plateL if self.plateL is not None else self.eeL)
        parent_state = p.getLinkState(self.urL, parent_link, computeForwardKinematics=True)
        ...
        child_pos, child_orn = p.getBasePositionAndOrientation(self.top_band_id)
        inv_pos, inv_orn = p.invertTransform(parent_pos, parent_orn)
        child_local_pos, child_local_orn = p.multiplyTransforms(inv_pos, inv_orn, child_pos, child_orn)
        seed_cid = p.createConstraint(
            self.urL, parent_link,
            self.top_band_id, -1,
            p.JOINT_FIXED,
            [0, 0, 0],
        ...
        for _ in range(35):
            self.set_left_gripper_opening(float(self.soft_grasp_closed_opening))
            p.stepSimulation()
        try:
            p.removeConstraint(seed_cid)
        except Exception:
            pass
        for _ in range(20):
            self.set_left_gripper_opening(float(self.soft_grasp_closed_opening))
            p.stepSimulation()
    def _spawn_soft_eccentric_ballast(self):
        verts, _ = self.get_soft_verts(self.sack_id)
        if len(verts) == 0 or self.sack_ballast_total_mass <= 0.0:
            return []

        center = verts.mean(axis=0)
        candidates = list(dict.fromkeys(self.sack_bot_indices + self.sack_mid_indices))
        cand_pos = verts[candidates]
        side_sign = cand_pos[:, 1] - center[1]
        low_mask = cand_pos[:, 2] < (center[2] - 0.012)
        side_mask = side_sign < -0.004
        use_mask = np.logical_and(low_mask, side_mask)
        if not np.any(use_mask):
            use_mask = cand_pos[:, 2] < center[2]
        chosen_candidates = [int(candidates[i]) for i in np.where(use_mask)[0].tolist()]
        if len(chosen_candidates) < 8:
            chosen_candidates = list(map(int, candidates))

        anchor_nodes = self._sample_indices_even(chosen_candidates, max(6, int(self.sack_ballast_count) * 6))
        anchor_nodes = [int(n) for n in anchor_nodes]
        anchor_positions = verts[anchor_nodes]
        base_pos = anchor_positions.mean(axis=0)
        base_pos = base_pos + np.array([0.0, -0.006, -0.010], dtype=np.float32)

        col = p.createCollisionShape(p.GEOM_SPHERE, radius=float(self.sack_ballast_radius))
        vis = p.createVisualShape(p.GEOM_SPHERE, radius=float(self.sack_ballast_radius), rgbaColor=self.sack_ballast_rgba)
        bid = p.createMultiBody(
            baseMass=float(self.sack_ballast_total_mass),
            baseCollisionShapeIndex=col,
            baseVisualShapeIndex=vis,
        ...
        p.changeDynamics(
            ...
            rollingFriction=0.0,
            linearDamping=float(self.sack_ballast_linear_damping),
            angularDamping=float(self.sack_ballast_angular_damping),
            restitution=0.0,
        )
        ...
        for node_idx in anchor_nodes:
            ...
            cid = p.createSoftBodyAnchor(self.sack_id, node_idx, bid, -1)
            self.soft_ballast_anchor_ids.append(cid)
        return [bid]

    def _anchor_soft_sack_to_left_gripper(self):
        self.soft_anchor_ids = []
        verts, _ = self.get_soft_verts(self.sack_id)
        if len(verts) == 0:
            return
        top_indices = list(self.sack_top_indices)
        top_positions = verts[top_indices]
        left_pad = self._find_link_or_none(self.urL, ["left_inner_finger_pad", "left_inner_finger"])
        right_pad = self._find_link_or_none(self.urL, ["right_inner_finger_pad", "right_inner_finger"])
        center_link = self.plateL if self.plateL is not None else self.eeL
        left_pos = np.array(p.getLinkState(self.urL, left_pad, computeForwardKinematics=True)[4], dtype=np.float32) if left_pad is not None else self._left_gripper_pad_center_world()
        right_pos = np.array(p.getLinkState(self.urL, right_pad, computeForwardKinematics=True)[4], dtype=np.float32) if right_pad is not None else self._left_gripper_pad_center_world()
        center_pos = self._left_gripper_pad_center_world()

        span = right_pos - left_pos
        span_norm = float(np.linalg.norm(span))
        if span_norm > 1e-8:
            span_dir = span / span_norm
        else:
            span_dir = np.array([0.0, 1.0, 0.0], dtype=np.float32)
        rel = top_positions - center_pos[None, :]
        side_coord = rel @ span_dir
        left_candidates = np.where(side_coord <= 0.0)[0].tolist()
        right_candidates = np.where(side_coord >= 0.0)[0].tolist()
        if not left_candidates:
            left_candidates = list(range(len(top_indices)))
        if not right_candidates:
            right_candidates = list(range(len(top_indices)))
        left_order = sorted(left_candidates, key=lambda i: float(np.linalg.norm(top_positions[i] - left_pos)))
        right_order = sorted(right_candidates, key=lambda i: float(np.linalg.norm(top_positions[i] - right_pos)))
        center_order = np.argsort(np.linalg.norm(top_positions - center_pos[None, :], axis=1))
        used = set()
        per_pad = int(self.soft_anchor_count_per_pad)
        for idx in left_order[:per_pad]:
            node = int(top_indices[int(idx)])
            used.add(node)
            self.soft_anchor_ids.append(p.createSoftBodyAnchor(self.sack_id, node, self.urL, int(left_pad if left_pad is not None else center_link)))
        for idx in right_order[:per_pad]:
            node = int(top_indices[int(idx)])
            if node in used:
                continue
            used.add(node)
            self.soft_anchor_ids.append(p.createSoftBodyAnchor(self.sack_id, node, self.urL, int(right_pad if right_pad is not None else center_link)))
        center_added = 0
        for idx in center_order:
            ...
            if node in used:
                continue
            used.add(node)
            self.soft_anchor_ids.append(p.createSoftBodyAnchor(self.sack_id, node, self.urL, int(center_link)))
            center_added += 1
            if center_added >= int(self.soft_anchor_center_count):
                break
        for _ in range(45):
            self.set_left_gripper_opening(float(self.soft_grasp_closed_opening))
            p.stepSimulation()
    ...
    def get_soft_verts(self, soft_id):
        try:
            mesh = p.getMeshData(soft_id, flags=p.MESH_DATA_SIMULATION_MESH)
        except Exception:
    ...
    def _capture_softbody_vertices(self, soft_id):
        verts, _ = self.get_soft_verts(soft_id)
        return verts if len(verts) else None

    def _best_fit_rotation(self, rest_offsets, current_offsets):
        rest = np.array(rest_offsets, dtype=np.float32)
        cur = np.array(current_offsets, dtype=np.float32)
        if len(rest) == 0 or len(cur) == 0 or rest.shape != cur.shape:
    ...
    def _get_sack_payload_com(self):
        bodies = []
        sack_state = self._get_sack_state()
        if sack_state is not None:
            bodies.append((None, float(self.sack_soft_mass), np.array(sack_state["center"], dtype=np.float32)))
        for bid in getattr(self, 'sack_content_ids', []):
            ...
            if mass > 0.0:
                ...
                bodies.append((bid, mass, np.array(pos, dtype=np.float32)))
        total = 0.0
        accum = np.zeros(3, dtype=np.float32)
        for _, mass, pos in bodies:
            accum += mass * pos
            total += mass
        if total <= 1e-8:
            return None
    ...
    def _get_sack_state(self):
        ...
        rest = self.sack_rest_vertices_scaled - self.sack_rest_vertices_scaled.mean(axis=0)
        current = verts - center
        rot = self._best_fit_rotation(rest, current)
        quat = np.array(self._rotation_matrix_to_quaternion(rot), dtype=np.float32)
